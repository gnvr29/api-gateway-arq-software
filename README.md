...
API Gateway baseado em Nginx que roteia tráfego para os microserviços de **Alunos** e **Cursos**, com TLS, rate limiting, load balancing e stack de observabilidade (Loki + Promtail + Grafana).
...
Client (HTTPS)
     ...
     ▼
┌──────────────┐
│  Nginx (443) │ ← TLS termination, rate limiting, routing
└──────┬───────┘
       │
...
│   │   └── gateway.conf            # Server block, rotas, TLS
│   ├── snippets/
...
│   └── ssl/
│       ├── server.crt              # Certificado TLS (gitignored)
│       └── server.key              # Chave privada (gitignored)
├── postgres/
│   └── init/
...
- Portas disponíveis: `80`, `443`, `3000`, `3100`, `5432`

---

## Como Rodar

### 1. Gerar certificados SSL (apenas uma vez)

```bash
./infra/scripts/setup-ssl.sh
```

O script detecta automaticamente se você tem `mkcert` instalado (gera certs trusted pelo sistema, sem warnings no browser) ou faz fallback para `openssl` (self-signed, browsers alertam mas funciona).

Para instalar `mkcert` (recomendado):
```bash
brew install mkcert
```

Para regenerar os certificados:
```bash
./infra/scripts/setup-ssl.sh --force
```
...
curl -k https://localhost/api/alunos

# Criar um aluno
curl -k -X POST https://localhost/api/alunos \
  -H "Content-Type: application/json" \
  -d '{"nome": "Gabriel", "email": "gabriel@test.com"}'

# Listar cursos
curl -k https://localhost/api/cursos

# Criar um curso
curl -k -X POST https://localhost/api/cursos \
  -H "Content-Type: application/json" \
  -d '{"nome": "Arquitetura de Software", "descricao": "Patterns e práticas"}'
```

> **Nota:** O `-k` desabilita verificação do certificado (necessário com self-signed). Com `mkcert`, pode remover.
...
### TLS Termination
- TLS 1.2 e 1.3
- Cipher suite moderno (ECDHE + AES-GCM)
- HSTS habilitado (força browsers a usar HTTPS)
- HTTP (porta 80) redireciona automaticamente para HTTPS (443)
...
2. Ir em **Explore** → selecionar datasource **Loki**
3. Queries úteis:
   ```logql
   {job="nginx"} | json
...
| Cert warning no browser | Esperado com self-signed. Usar `mkcert` para resolver |
...
- [ ] Configurar CORS nos endpoints que precisam (`include /etc/nginx/snippets/cors.conf`)
- [ ] Dashboard pré-configurado no Grafana (importar JSON)
- [ ] Let's Encrypt para certificados reais em produção
